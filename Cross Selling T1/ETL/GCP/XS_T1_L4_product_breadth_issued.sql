...
Destination table: `tvlk-data-user-dev.xs_playground.XS_T1_L4_product_breadth_issued
Output Field: 
    * dt,
    * period,
    * metric,
...
-- CREATE OR REPLACE TABLE `tvlk-data-user-dev.xs_playground.XS_T1_L4_product_breadth_issued`
-- PARTITION BY (dt) AS

DELETE FROM 
  `tvlk-data-user-dev.xs_playground.XS_T1_L4_product_breadth_issued`
WHERE 
  ((EXTRACT(day
      FROM
        CURRENT_DATE()) = 1
...
INSERT INTO `tvlk-data-user-dev.xs_playground.XS_T1_L4_product_breadth_issued`
...
SELECT
  ...
  channel,
  section,
  ROUND(AVG(num_xs_prod), 6) AS avg_xs_prod_issuance
FROM (
  SELECT
