...
Destination table: `tvlk-data-user-dev.xs_playground.XS_T1_L4_all_addons_revenue_in_local_currency`
Output Field: 
    * transaction_dt,
    * period,
    * metric,
...
-- CREATE OR REPLACE TABLE `tvlk-data-user-dev.xs_playground.XS_T1_L4_all_addons_revenue_in_local_currency`
...
SELECT
  ...
  SUM(gbv_lcy) AS gbv_lcy,
  SUM(net_rev_lcy) AS net_rev_lcy,
  ROUND(SAFE_DIVIDE(SUM(gbv_lcy), COUNT(DISTINCT profile_id)), 4) AS avg_gbv_lcy,
  ROUND(SAFE_DIVIDE(SUM(net_rev_lcy), COUNT(DISTINCT profile_id)), 4) AS avg_net_rev_lcy,
  ROUND(SAFE_DIVIDE(SUM(gbv_lcy), COUNT(DISTINCT booking_id)), 4) AS avg_gbv_lcy_per_bid,
  ROUND(SAFE_DIVIDE(SUM(net_rev_lcy), COUNT(DISTINCT booking_id)), 4) AS avg_net_rev_lcy_per_bid
FROM
  base_data_monthly
GROUP BY
  1,
