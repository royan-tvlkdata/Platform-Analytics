...
Destination table: `tvlk-data-user-dev.xs_playground.XS_T1_L3_transaction_cross_products_per_user`
Output Field: 
    * event_month 
    * profile_id  
...
-- CREATE OR REPLACE TABLE `tvlk-data-user-dev.xs_playground.XS_T1_L3_transaction_cross_products_per_user`
-- PARTITION BY (event_month) AS 

WITH
  base AS (
  SELECT
    DISTINCT DATE_TRUNC(transaction_dt, month) AS event_month,
    profile_id,
...
--     DATE_TRUNC(transaction_dt, MONTH) BETWEEN DATE_ADD(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL -2 YEAR)
